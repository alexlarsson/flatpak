...
ensure_safe_staging_permissions (OstreeRepo *repo, GError **error)
{
  ...
  if (!glnx_opendirat (ostree_repo_get_dfd (repo), "tmp", TRUE, &tmp_fd, error))
    return FALSE;

  if (!glnx_dirfd_iterator_init_at (tmp_fd, ".", TRUE, &tmp_iter, error))
    return FALSE;
  ...
  while (TRUE)
    {
      ...
      if (dent->d_type == DT_DIR && g_str_has_prefix (dent->d_name, "staging-") &&
          !ensure_safe_objdir (tmp_fd, dent->d_name, error))
        {
          glnx_shutil_rm_rf_at (tmp_fd, dent->d_name, NULL, NULL);
          return FALSE;
        }
    }
